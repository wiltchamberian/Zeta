...
void test_cnn_linear() {
    ...
    // 第一层 2 -> 2
    Linear* layer1 = network.CreateLayer<Linear>(2, 2);
    ...
    // 第二层 2 -> 1
    Linear* layer2 = network.CreateLayer<Linear>(2, 1);
...
void test_cnn_tictac() {
    ...
    auto fully1 = network.CreateLayer<Linear>(9, 9);
    for (int i = 0; i < 9; ++i) {
        for (int j = 0; j < 9; ++j) {
    ...
    auto fully2 = network.CreateLayer<Linear>(9, 1);
    fully2->alpha = 1;
    for (int i = 0; i < 9; ++i) {
        for (int j = 0; j < 1; ++j) {
